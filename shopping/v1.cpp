...
struct Video {
    ...
    long long playCount;
    int order; // appearance order in the input
};
...
void extractHashtags(const std::string& text, std::vector<std::string>& hashtags) {
    std::regex hashtagRegex("#([\\w\\u0080-\\uFFFF]+)");
    std::sregex_iterator iter(text.begin(), text.end(), hashtagRegex);
    std::sregex_iterator end;
    while (iter != end) {
        hashtags.push_back(iter->str(1));
        ++iter;
    }
}
