...
void processFile(const std::string& inputFile, const std::string& outputFile, const std::string& mode) {
    ...
    while (getline(in, line)) {
        ...
        if (videoId.empty()) {
            std::cerr << "[DEBUG] Skipping video due to missing ID. text: " << text << " playCount: " << playCount << std::endl;
            continue;
        }
        ...
        if (mode == "hashtag") {
            ...
            for (std::unordered_map<std::string, int>::iterator it = tagFrequency.begin(); it != tagFrequency.end(); ++it) {
                const std::string& tag = it->first;
                int freq = it->second;
                std::cerr << "#" << tag << "(" << freq << ") ";
                hashtagMap[tag].count += freq;
                hashtagMap[tag].totalViews += playCount * freq;
    ...
    if (mode == "hashtag") {
        ...
        for (int i = 0; i < 20 && !pq.empty(); ++i) {
            ...
            for (int j = 0; j < std::min(3, (int)data.videos.size()); ++j) {
                if (!data.videos[j].coverUrl.empty())
                    out << "cover url: " << data.videos[j].coverUrl << "\n";
                if (!data.videos[j].webVideoUrl.empty())
        ...
        for (int i = 0; i < 20 && !pq.empty(); ++i) {
            ...
            for (int j = 0; j < std::min(3, (int)top.videos.size()); ++j) {
                if (!top.videos[j].coverUrl.empty())
                    out << "cover url: " << top.videos[j].coverUrl << "\n";
                if (!top.videos[j].webVideoUrl.empty())
