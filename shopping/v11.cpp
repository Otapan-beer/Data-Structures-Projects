...
void processFile(const std::string& inputFile, const std::string& outputFile, const std::string& mode) {
    ...
    while (getline(in, line)) {
        ...
        if (mode == "hashtag") {
            ...
            for (int i = 0; i < (int)tags.size(); ++i) {
                hashtagMap[tags[i]].count++;
                hashtagMap[tags[i]].totalViews += playCount;
                hashtagMap[tags[i]].videos.push_back(v);
            }
        } else if (mode == "sound") {
    ...
    if (mode == "hashtag") {
        ...
        for (int i = 0; i < 20 && !pq.empty(); ++i) {
            ...
            int shown = 0;
            for (int j = 0; j < (int)data.videos.size() && shown < 3; ++j) {
