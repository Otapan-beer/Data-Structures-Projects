    ...
    while (getline(in, line)) {
        ...
        if (mode == "hashtag") {
            ...
            for (std::vector<std::string>::iterator it = tags.begin(); it != tags.end(); ++it) {
                if (uniqueTags.find(*it) != uniqueTags.end()) {
                    std::cerr << "[DEBUG] Duplicate hashtag in one video: #" << *it << " (video ID: " << videoId << ")\n";
                }
                uniqueTags.insert(*it);
            }
            std::cerr << "[DEBUG] Video " << videoId << " uses hashtags: ";
            for (std::unordered_set<std::string>::iterator it = uniqueTags.begin(); it != uniqueTags.end(); ++it) {
                std::cerr << "#" << *it << " ";
                hashtagMap[*it].count++;
