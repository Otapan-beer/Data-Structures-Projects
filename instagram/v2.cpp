        ...
        if (type == "likes") {
            if (postToOwner.find(target) != postToOwner.end()) {
                ...
                if (owner == targetUser && shouldNotify(owner, "likes"))
                    notifications.push_back(unique_ptr<Notification>(new LikeNotification(actor)));
            }
        } else if (type == "tags") {
            if (target == targetUser && shouldNotify(target, "tags"))
                notifications.push_back(unique_ptr<Notification>(new TagNotification(actor)));
        } else if (type == "comments_on") {
            if (postToOwner.find(target) != postToOwner.end()) {
                string owner = postToOwner[target];
                if (owner == targetUser && shouldNotify(owner, "comments"))
                    notifications.push_back(unique_ptr<Notification>(new CommentNotification(actor)));
            }
        } else if (type == "follows") {
            if (target == targetUser && shouldNotify(target, "follows"))
                notifications.push_back(unique_ptr<Notification>(new FollowNotification(actor)));
        } else if (type == "messageRequests") {
            if (target == targetUser && shouldNotify(target, "messages"))
                notifications.push_back(unique_ptr<Notification>(new MessageRequestNotification(actor)));
        }
    }
