...
int main(int argc, char* argv[]) {
    ...
    auto getNotificationPriority = [](const Notification* notif) -> int {
        string msg = notif->format();
        if (msg.find("wants to send you a message") != string::npos) return 1;
        if (msg.find("tagged you in a post") != string::npos) return 2;
        if (msg.find("commented on your post") != string::npos) return 3;
        if (msg.find("liked your post") != string::npos) return 4;
        if (msg.find("started following you") != string::npos) return 5;
        return 6; // fallback
    };

    // Sort by priority then by original event order
