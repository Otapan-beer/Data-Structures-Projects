...
struct OrderedNotification {
    ...
    OrderedNotification(int o, const string& a, unique_ptr<Notification> n)
        : order(o), actor(a), notif(std::move(n)) {}
};
...
int getNotificationPriority(const Notification* notif) {
    ...
    if (dynamic_cast<const LikeNotification*>(notif)) return 2;
...
int main(int argc, char* argv[]) {
        ...
        if (type == "likes") {
            if (postToOwner.find(target) != postToOwner.end()) {
                ...
                if (owner == targetUser && shouldNotify(owner, "likes")) {
                    notifications.push_back(OrderedNotification(lineIndex, actor, unique_ptr<Notification>(new LikeNotification(actor))));
                }
            }
        } else if (type == "tags") {
            if (target == targetUser && shouldNotify(target, "tags")) {
                notifications.push_back(OrderedNotification(lineIndex, actor, unique_ptr<Notification>(new TagNotification(actor))));
            }
        } else if (type == "comments_on") {
            if (postToOwner.find(target) != postToOwner.end()) {
                string owner = postToOwner[target];
                if (owner == targetUser && shouldNotify(owner, "comments")) {
                    notifications.push_back(OrderedNotification(lineIndex, actor, unique_ptr<Notification>(new CommentNotification(actor))));
                }
            }
        } else if (type == "follows") {
            if (target == targetUser && shouldNotify(target, "follows")) {
                notifications.push_back(OrderedNotification(lineIndex, actor, unique_ptr<Notification>(new FollowNotification(actor))));
            }
        } else if (type == "messageRequests") {
            if (target == targetUser && shouldNotify(target, "messages")) {
                notifications.push_back(OrderedNotification(lineIndex, actor, unique_ptr<Notification>(new MessageRequestNotification(actor))));
            }
        }
        lineIndex++;
    }
    ...
    for (const auto& n : notifications) {
        outFile << n.notif->format() << endl;
        cerr << "DEBUG: Notification written: " << n.notif->format() << endl;
    }
