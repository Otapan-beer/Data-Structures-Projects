...
private:
    ...
    std::shared_ptr<CommentNode> parseCommentLine(const std::string& line) {
        ...
        if (cid.empty()) {
            logError("Missing comment_id in line: " + line);
            return nullptr;
        }
        try {
            ...
            logInfo("Parsed comment: " + cid);
            return node;
        } catch (const std::exception& e) {
            logError("Exception parsing comment " + cid + ": " + e.what());
            return nullptr;
        }
    }
        ...
        while (getline(file, line)) {
            ...
            if (node->parent_comment_id.empty()) {
                ...
                if (comment_map.count(node->parent_comment_id)) {
                    ...
                    logError("Parent not found for reply: " + node->comment_id);
                }
            }
            count++;
        }
        logInfo("Loaded " + std::to_string(count) + " comments.");
    }
        ...
        while (getline(file, line)) {
            ...
            if (cmd == "reply_to_comment") {
                ...
                if (comment_map.count(parent_id)) {
                    ...
                    logError("Parent comment not found for reply: " + parent_id);
                }
                ...
                if (comment_map.count(cid)) {
                    ...
                    logError("Comment not found for like: " + cid);
                }
                ...
                if (comment_map.count(cid)) {
                    ...
                    out.close();
                    logInfo("Wrote comment tree to: " + output_file);
                } else {
                    logError("Comment ID not found for display: " + cid);
                }
            } else {
