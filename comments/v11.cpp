...
private:
    ...
    void displayCommentHelper(const std::shared_ptr<CommentNode>& node, std::ofstream& out, int depth = 0) {
        ...
        // Show immediate child reply count only at root level
        if (depth == 0 && !node->children.empty()) {
            size_t visible_children = 0;
            for (const auto& child : node->children) {
                if (!deleted_comments.count(child->comment_id)) visible_children++;
            }
            if (visible_children > 0) {
                out << indent << visible_children << (visible_children == 1 ? " reply\n" : " replies\n");
            }
        }
    ...
    int countTotalReplies(const std::shared_ptr<CommentNode>& node) {
        int count = 0;
        for (const auto& child : node->children) {
            if (!deleted_comments.count(child->comment_id)) {
                count += 1 + countTotalReplies(child);
            }
        }
        return count;
    }
