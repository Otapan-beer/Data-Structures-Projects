...
private:
    ...
    int countImmediateReplies(const std::shared_ptr<CommentNode>& node) {
        int count = 0;
        for (std::vector<std::shared_ptr<CommentNode>>::const_iterator it = node->children.begin(); it != node->children.end(); ++it) {
            if (!deleted_comments.count((*it)->comment_id)) {
                count++;
    ...
    void displayCommentHelper(const std::shared_ptr<CommentNode>& node, std::ofstream& out, int depth = 0, bool is_root = false) {
        ...
        if (is_root) {
            int total_descendant_replies = countTotalReplies(node);
            if (total_descendant_replies > 0) {
                out << indent << total_descendant_replies << (total_descendant_replies == 1 ? " reply\n" : " replies\n");
            }
        } else {
            int immediate_children = countImmediateReplies(node);
            if (immediate_children > 0) {
                out << indent << immediate_children << (immediate_children == 1 ? " reply\n" : " replies\n");
            }
        }
        ...
        while (getline(file, line)) {
            ...
            if (cmd == "reply_to_comment") {
                ...
                if (comment_map.count(cid) && !deleted_comments.count(cid) && !output_written) {
                    ...
                    std::unordered_map<std::string, std::shared_ptr<CommentNode>>::const_iterator it = comment_map.find(cid);
                    if (it != comment_map.end()) {
                        displayCommentHelper(it->second, out, 0, true);
                    }
                    out.close();
                    output_written = true;
                }
