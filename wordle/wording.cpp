...
void fillEmptySpaces(std::vector<std::string>& grid, const std::set<std::string>& forbiddenWords) {
    ...
    for (int row = 0; row < height; ++row) {
        for (int col = 0; col < width; ++col) {
            if (grid[row][col] == '.') {
                for (char letter = 'a'; letter <= 'z'; ++letter) { //apparently we can iterate through characters like this because a is set as the smallest and z the highest :D
                    grid[row][col] = letter;
...
void fillEmptySpacesAll(std::vector<std::string>& grid, const std::set<std::string>& forbiddenWords, std::set<std::vector<std::string>>& uniqueSolutions) {
    std::set<std::vector<std::string>> uniqueBoards;
    int height = grid.size();
    int width = grid[0].size();
    for (int row = 0; row < height; ++row) {
        for (int col = 0; col < width; ++col) {
...
void findAllSolutions(std::vector<std::string>& grid, const std::vector<std::string>& words, int index, int width, int height, std::set<std::vector<std::string>>& uniqueSolutions, const std::set<std::string>& forbiddenWords) {
    ...
    if (index == words.size()) {
        fillEmptySpacesAll(grid, forbiddenWords, uniqueSolutions);
        if(!isForbiddenWordPresent(grid, forbiddenWords)) {
