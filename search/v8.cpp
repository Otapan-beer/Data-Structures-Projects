...
std::string extractSnippet(const std::string& bodyContent, const std::string& query) {
    std::string snip = ""; // Initialize the snippet string
    if (bodyContent.find(query) != std::string::npos) {
        // If the query is found in the body content, extract a 120-character snippet starting from the query
        snip = bodyContent.substr(bodyContent.find(query), 120);
    } else {
        // If the query is not found, search for individual words in the query
        std::istringstream iss(query);
        std::string word;
        while (iss >> word) {
            if (bodyContent.find(word) != std::string::npos) {
                // If a word is found, extract a 120-character snippet starting from the word
                snip = bodyContent.substr(bodyContent.find(word), 120);
                break;
            }
        }
    }
    std::cout << "Snippet: " << snip << std::endl; // Debugging statement
...
std::string removeLinks(const std::string& bodyContent) {
    // Remove all <a> tags (links) from the body content using regex
...
void searchAndRank(const std::string& query, const std::map<std::string, std::string>& documents, const std::map<std::string, std::list<std::string>>& outgoingLinks, std::ofstream& outFile) {
    ...
    // Debugger output: Print scores for each document to console
