...
void StudentTest(){
    std::cout << "Running additional student test cases..." << std::endl;

    //multiply_by_coefficient
    Matrix m1(2, 2, 3.0);
    m1.multiply_by_coefficient(2.0);
    double val;
    assert(m1.get(0, 0, val) && double_compare(val, 6.0));
    assert(m1.get(1, 1, val) && double_compare(val, 6.0));
    std::cout << "Passed multiply_by_coefficient test." << std::endl;
    ...
    assert(m2_transposed.num_rows() == 3);
    assert(m2_transposed.num_cols() == 2);
    assert(m2_transposed.get(0, 1, val) && double_compare(val, 4));
    std::cout << "Passed transpose test." << std::endl;
    ...
    double* col_data = m3.get_col(0);
    assert(col_data[0] == 1 && col_data[1] == 2 && col_data[2] == 3);
    delete[] col_data;
    std::cout << "Passed get_col test." << std::endl;

    //zero rows and cols
    Matrix empty_matrix;
    assert(empty_matrix.num_rows() == 0);
    assert(empty_matrix.num_cols() == 0);
    std::cout << "Passed empty matrix test." << std::endl;

    //wuartering with odd dimensions
    Matrix odd_matrix(3, 3, 1);
    Matrix* quarters = odd_matrix.quarter();
    assert(quarters != nullptr);
    assert(quarters[0].num_rows() == 2 && quarters[0].num_cols() == 2);
    delete[] quarters;
    std::cout << "Passed quartering test." << std::endl;

    std::cout << "All student test cases passed!" << std::endl;
}
